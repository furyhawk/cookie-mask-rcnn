...
(`src/cookie_mask_rcnn/general_utils.py`)
...
a Dockerfile (`docker/cookie-mask-rcnn-data-prep.Dockerfile`)
...
=== "Linux/macOS"
    ...
    $ docker build \
        -t asia.gcr.io/$GCP_PROJECT_ID/teck/data-prep:0.1.0 \
        -f docker/cookie-mask-rcnn-data-prep.Dockerfile \
        --platform linux/amd64 .
    $ docker push asia.gcr.io/$GCP_PROJECT_ID/teck/data-prep:0.1.0
    ```
...
=== "Linux/macOS"
    ...
    $ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/process-data.yml \
        -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/teck/data-prep:0.1.0" \
        ...
        -P WORKING_DIR="/home/aisg/cookie-mask-rcnn" \
...
In the module `src/cookie_mask_rcnn/general_utils.py`,
...
!!! important
    ...
    following name: `gs://cookie-mask-rcnn-artifacts`.
    ...
    will be
    `gs://cookie-mask-rcnn-artifacts/mlflow-tracking-server`.

=== "Local Machine / Polyaxon VSCode Terminal"

    ```bash
    $ gsutil mb -p cvhub-312105 -c STANDARD -l ASIA-SOUTHEAST1 -b on gs://cookie-mask-rcnn-artifacts
    Creating gs://cookie-mask-rcnn-artifacts/...
    ```
...
`gs://cookie-mask-rcnn-artifacts/mlflow-tracking-server/teck`.
...
=== "Linux/macOS"
    ...
    ```bash
    $ docker build \
        -t asia.gcr.io/$GCP_PROJECT_ID/teck/model-train:0.1.0 \
        -f docker/cookie-mask-rcnn-model-training-gpu.Dockerfile \
        --platform linux/amd64 .
    $ docker push asia.gcr.io/$GCP_PROJECT_ID/teck/model-train:0.1.0
    ```
...
=== "Linux/macOS"
    ...
    $ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/train-model-gpu.yml \
        -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/teck/model-train:0.1.0" \
