...
- Artifacts for specific runs will be uploaded to a directory with a
  ...
  `gs://cookie-mask-rcnn-artifacts/mlflow-tracking-server/teck/<MLFLOW_EXPERIMENT_UUID>`.
...
`src/cookie_mask_rcnn_fastapi/config.py`:
```python
...
...
=== "Linux/macOS"
    ...
    $ docker build \
        -t asia.gcr.io/$GCP_PROJECT_ID/teck/fastapi-server:0.1.0 \
...
=== "Linux/macOS"
    ...
    $ docker run --rm -p 8080:8080 \
        ...
        asia.gcr.io/$GCP_PROJECT_ID/teck/fastapi-server:0.1.0
    ```
...
```bash
$ docker push asia.gcr.io/$GCP_PROJECT_ID/teck/fastapi-server:0.1.0
```
...
=== "Local Machine"
    ...
    ```bash
    $ kubectl port-forward service/fastapi-server-teck-svc 8080:8080 --namespace=polyaxon-v1
...
!!! attention
    ...
    === "Local Machine"
        ...
        ```bash
        $ kubectl delete fastapi-server-teck-deployment --namespace=polyaxon-v1
        $ kubectl delete fastapi-server-teck-svc --namespace=polyaxon-v1
        ```
