...
ARG REPO_DIR="."
ARG CONDA_ENV_FILE="cookie-mask-rcnn-conda-env.yml"
ARG CONDA_ENV_NAME="cookie-mask-rcnn"
...
ARG PRED_MODEL_GCS_URI="gs://cookie-mask-rcnn-artifacts/mlflow-tracking-server/teck/$PRED_MODEL_UUID"
...
COPY $REPO_DIR cookie-mask-rcnn
...
RUN $CONDA_BIN env create -f cookie-mask-rcnn/$CONDA_ENV_FILE && \
    $CONDA_BIN init bash && \
    $CONDA_BIN clean -a -y && \
    echo "source activate $CONDA_ENV_NAME" >> "$HOME_DIR/.bashrc"

WORKDIR $HOME_DIR/cookie-mask-rcnn
